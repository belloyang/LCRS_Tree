...
public class LCRS_Test{
	...
	public static LCRS_Tree tree;
	
	
	
	private static class UpdateRoutine implements Runnable
	{
		//public static LCRS_Tree lcrs_tree;
	...
	public static void main(String[] args)
	{
		...
		if(args.length>=1)
		{
			LCRS_Test.nThread = Integer.parseInt(args[0]);
		}
		
		if(args.length>=2)
		{
			LCRS_Test.mode = Integer.parseInt(args[1]);
		}
		
		System.out.println("Benchmark Setting for LCRS_Tree Algorithm:\n"+nThread+" Threads are runing in mode:"+
				mode);
		switch(mode)
		{
			case 0:{
		...
		System.out.println("LCRS_Tree is initilized! "+count+" nodes are inserted in the tree!");
		...
		try(PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter("LCRS_Test."+args[0]+".out", true)))) {
